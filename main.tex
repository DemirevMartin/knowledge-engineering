...
\begin{itemize}
    ...
    \item \textbf{Environmental Impact Factors:} Quantitative instances derived from two scientific sources. Primary CO$_2$e values come from the Wolfram Food Carbon Footprint dataset (538 foods, compiled from peer-reviewed LCA studies including Poore \& Nemecek 2018). For 9 ingredients absent from Wolfram, fallback values were sourced from the Our World in Data GHG Emissions per kg dataset (Poore \& Nemecek 2018 via OWID). Each instance is a CO$_2$e rate (kg CO$_2$e/kg of food) mapped to a canonical ingredient entity.
...
\begin{itemize}
    \item \textbf{Nodes (Entity Instances):} The graph contains several distinct classes of nodes:
    \begin{itemize}
        \item \textit{Recipe Nodes:} Master entities containing high-level metadata and ratings.
        \item \textit{Ingredient Nodes:} Building blocks of the recipes, mapped to scientific impact data.
        \item \textit{Instructional Steps:} Ordered procedural instances representing the temporal knowledge required for recipe execution.
        \item \textit{Cuisine/Category Nodes:} Organizational instances used for regional classification (e.g., Pakistani, Indian, Bangladeshi).
    \end{itemize}
    \item \textbf{Edges (Relationship Instances):} The graph utilizes directed relationship types to link these nodes:
    \begin{itemize}
        \item \textit{Composition (Recipe $\rightarrow$ Ingredient):} A weighted relationship
        representing the inclusion of an ingredient, qualified by a \texttt{grams} attribute
        expressing the quantity in metric mass (g), grounded via USDA SR Legacy portion data.
        \item \textit{Classification (Ingredient $\rightarrow$ Food Category):} A semantic edge mapping ingredients to broader groups (e.g., Dairy, Grains) to facilitate environmental proxy modeling for specialized regional items.
        \item \textit{Procedural Linkage (Recipe $\rightarrow$ Steps):} A sequential relationship connecting a recipe to its constituent preparation instructions.
        \item \textit{Sustainability Mapping (Ingredient $\rightarrow$ Carbon Metric):} An attribute-link connecting canonical ingredients to validated greenhouse gas emission factors.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Cuisine/Category Nodes:} 5 primary regional categories (Indian,
    Pakistani, Bangladeshi, Afghan, Fusion).
    \item \textbf{HAS\_INGREDIENT Edges:} 103,908 directed edges, one per ingredient
    line item, each carrying \texttt{grams}, \texttt{g\_per\_unit}, and
    \texttt{usda\_tier} attributes.
    \item \textbf{Instructional Step Nodes:} 77,914 individual procedural steps.
    \item \textbf{Nutritional Profile Nodes:} 9,997 instances, one per recipe,
    containing macro and micronutrient attributes.
    \item \textbf{CO$_2$e Factor Instances:} 58 validated emission rates
    (kg~CO$_2$e/kg of food), one per canonical ingredient.
...
In total, the Knowledge Graph architecture manages approximately 202,000 nodes and
relational edges across culinary, nutritional, and environmental domains.
...
\begin{itemize}
    ...
    \item \textbf{Sequential Instructional Mapping:} The procedural link between a recipe and its preparation logic is explicitly established in \texttt{recipe\_steps.csv}. Using the recipe\_id as a join key, the system maps recipes to a series of ordered instructional steps (step\_number), creating a temporal relationship within the graph.
...
\begin{itemize}
    ...
    \item \textbf{Heterogeneous Measurement Noise (partially resolved):} The \texttt{quantity} field contained non-computable strings (``to taste'', ``a dash'') alongside conflicting volumetric and mass scales (cups, grams, tablespoons, ml). This was addressed through a three-tier quantity grounding pipeline: (1) USDA SR Legacy lookup for empirically measured gram weights per culinary unit; (2) density-based conversion for liquid ingredients given in ml; (3) fixed fallback defaults for irreducible cases. Approximately 52.9\% of the ingredient rows were resolved with USDA SR Legacy; the remaining 47.1\% used fallback values. All conversion decisions are recorded in the \texttt{usda\_tier} column of \texttt{recipe\_ingredients\_grounded.csv}.
    ...
    \item \textbf{Relational Overlap (managed):} The 9,997 master recipe records are distributed across three relational tables (\texttt{recipes\_master.csv}, \texttt{recipe\_nutrition.csv}, \texttt{recipe\_steps.csv}). This structural redundancy was managed by using \texttt{recipe\_id} as a join key during graph construction, centralising all attributes under a single Recipe node.
\end{itemize}
...
The 538 food items in Dataset Wolfram represent a curated selection from the broader universe of foods evaluated in LCA literature. The selection appears to prioritise globally traded commodity foods and Western dietary staples. This introduces a systematic gap for this project: several South Asian ingredients (e.g., fenugreek leaves, curry leaves, regional spice blends) are absent from Wolfram's coverage, necessitating the OWID fallback described in Section 4.
...
\begin{itemize}
    \item \textbf{Dataset Foods~\cite{kaggle_southasian}:} The original Kaggle files (\texttt{recipe\_ingredients.csv}, \\ \texttt{recipe\_steps.csv}, \texttt{recipe\_nutrition.csv}, \texttt{recipes\_master.csv}) are retained unmodified. Attributes excluded from the current scope (e.g., step-by-step instructions, some categorical metadata) remain available for future use.
...
\begin{enumerate}
    ...
    \item \textbf{CO$_2$e Mapping:} Each canonical ingredient was assigned a CO$_2$e rate. The mapping rule was strict: every canonical ingredient must be assigned a value. Wolfram was queried first; OWID served as fallback for the 9 ingredients not covered by Wolfram. 100\% coverage was achieved.
    \item \textbf{Gram-Weight Grounding:} Each ingredient-quantity row was assigned a gram weight via USDA SR Legacy lookup (52.9\% of rows) or fallback defaults (47.1\%). Results are stored in \\ \texttt{recipe\_ingredients\_grounded.csv} with full provenance (\texttt{usda\_tier}, \texttt{g\_per\_unit}).
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Step 2: CO$_2$e Rate Assignment:} Each canonical ingredient was assigned a CO$_2$e rate in \texttt{main.ipynb}. Wolfram was queried first via \texttt{WOLFRAM\_MAP}. For the 9 ingredients not covered by Wolfram, the OWID GHG dataset was used via \texttt{OWID\_MAP}. This achieved 100\% CO$_2$e coverage across all 58 canonical ingredients.
...
\end{equation}
Results are stored in \texttt{recipe\_co2.csv} (one row per recipe, 9,997 rows). Across all recipes, the median CO$_2$e is 9.04\,kg CO$_2$eq and the mean is 13.68\,kg CO$_2$eq.
...
The raw \texttt{recipe\_ingredients.csv} contains 103,908 ingredient line items but only 58 distinct base ingredients, obscured by surface-form variation (e.g.\ ``chopped onions'', ``red onion'', ``medium onions'' all denote onion). Each of the 58 canonical names was matched to a CO$_2$e emission factor through a three-layer strategy, building up coverage as shown in Figure~\ref{fig:coverage}. A first pass matched names directly against the Wolfram food list (21 ingredients, 36\%). A manual synonym map (\texttt{WOLFRAM\_MAP}) then aligned culinary names to Wolfram's vocabulary --- for example ``ghee'' $\rightarrow$ butter, ``garam masala'' $\rightarrow$ spice --- lifting coverage to 50 ingredients (86\%). The remaining 8 ingredients absent from Wolfram (notably paneer and semolina) were resolved through an OWID GHG fallback (\texttt{OWID\_MAP}), reaching 100\% coverage of all 58 ingredients and all 103,908 rows. The full synonym and fallback maps are documented in Appendix~\ref{app:mappings}.
...
\begin{figure}[h]
    ...
    cover 36\% of canonical ingredients; the manual alias map raises this to 86\%; the OWID
...
\begin{equation}
    \text{co2e\_kg} = \sum_{i} \left( \text{qty\_amount}_i \times \text{g\_per\_unit}_i \times \frac{\text{co2\_kg\_per\_kg}_i}{1000} \right).
\end{equation}
This yields one score per recipe across all 9,997 recipes (median 9.04\,kg CO$_2$eq, mean 13.68\,kg CO$_2$eq), stored in \texttt{recipe\_co2.csv}. Because the gram weights are estimates rather than exact measurements, these values are best read as proportional indicators for comparative ranking rather than absolute measurements --- a framing carried through into the eco-score presented to the client. The assumptions underlying each conversion default are catalogued in Appendix~\ref{app:assumptions}.
...
Recipe ingredient names mapped to Wolfram canonical food names. Some are exact synonyms; others are deliberate proxies (e.g.\ ghee mapped to butter, garam masala mapped to the generic ``spice'' category).
...
\hline
lentils (dal) & lentil & eggs & egg \\
mint leaves & fresh mint & fenugreek leaves & fenugreek seed \\
fresh coriander & coriander seed & curry leaves & curry powder \\
tamarind paste & tamarind & chickpeas & chickpea \\
sesame seeds & sesame & fish & fishes \\
coriander & coriander seed & mustard seeds & mustard \\
green chili & chili pepper & almonds & almond \\
bell pepper & pepper & cashews & cashew \\
oil / cooking oil & sesame oil & raisins & raisin \\
all-purpose flour & wheat flour & bay leaf & herbs \\
red chili powder & chili pepper & peas & pea \\
...
Ingredients absent from the Wolfram trial file, mapped to OWID GHG categories. Only the entries that the pipeline actually fell back to are operative; the relevant proxies are noted.
...
\noindent\textbf{Approximations:} \textbf{paneer $\rightarrow$ Cheese} (paneer is a fresh cheese; the OWID Cheese category is a reasonable but not exact match, 23.88\,kg CO$_2$e/kg). \textbf{semolina $\rightarrow$ Wheat \& Rye} (OWID has no refined-semolina category; the whole-grain wheat category slightly overestimates emissions for refined semolina --- no closer category exists).
