...
Integration was a three-stage process designed to make culinary recipe data semantically compatible with scientific environmental data. The same end-to-end pipeline is summarised visually in Section~\ref{sec:solution} (Figure~\ref{fig:pipeline}); here we describe how the datasets were mapped together and report the resulting coverage and gram-weight statistics in full. Code is available in \texttt{main.ipynb} (CO$_2$e lookup pipeline) and \texttt{quantities.ipynb} (gram-weight grounding pipeline).

\paragraph{Stage 1: Ingredient canonicalisation and CO$_2$e rate assignment.}
A Python script in \texttt{main.ipynb} extracted all unique ingredient names from \texttt{recipe\_ingredients.csv}. The raw file contains 103,908 ingredient line items but only 58 distinct base ingredients, obscured by surface-form variation (e.g.\ ``chopped onions'', ``red onion'', ``medium onions'' all denote onion). Each of the 58 canonical names was matched to a CO$_2$e emission factor through a three-layer strategy, building up coverage as shown in Figure~\ref{fig:coverage}. A first pass matched names directly against the Wolfram food list (28 ingredients, 48\%). A manual synonym map (\texttt{WOLFRAM\_MAP}) then aligned culinary names to Wolfram's vocabulary --- for example ``ghee'' $\rightarrow$ butter, ``garam masala'' $\rightarrow$ spice --- lifting coverage to 54 ingredients (93\%). The remaining 4 ingredients absent from Wolfram (paneer, semolina, potato, tomato) were resolved through an OWID GHG fallback (\texttt{OWID\_MAP}), reaching 100\% coverage of all 58 ingredients and all 103,908 rows. The resulting mapping is recorded in \texttt{ingredient\_co2.csv} (canonical name, matched CO$_2$e value, and source). The full synonym and fallback maps are documented in Appendix~\ref{app:mappings}.
...
\paragraph{Stage 2: Gram-weight grounding via USDA SR Legacy.}
Emission factors are expressed per kilogram, but recipe quantities use culinary units (tablespoon, cup, ``to taste''). The same unit denotes very different masses depending on the ingredient: one tablespoon of fresh mint is roughly 2.5\,g while one tablespoon of minced beef is roughly 15\,g. Each ingredient-quantity entry in \texttt{recipe\_ingredients.csv} was therefore converted to grams in \texttt{quantities.ipynb} using a three-tier procedure, with the resulting split shown in Figure~\ref{fig:tier}. Tier~1 (52.9\% of rows) looks up an empirically measured gram weight in the USDA SR Legacy portion table (\texttt{food\_portion.csv} joined with \texttt{food.csv}), matching on ingredient keyword and unit type parsed from the \texttt{modifier} column, and taking the median of the five ``plainest'' (raw, unbranded) matches to avoid cooked or processed outliers. Tier~2 (17.2\%) covers quantities already given in absolute mass (g/kg), which pass through directly. Tier~3 (29.9\%) applies fixed fallback defaults where USDA has no matching entry, including density-based conversion for liquids given in ml and a conservative 5\,g default for irreducible cases such as ``to taste''. Every row records its resolution tier in \texttt{recipe\_ingredients\_grounded.csv} (\texttt{grams}, \texttt{g\_per\_unit}, \texttt{usda\_tier}) for full provenance.
...
With every row assigned both a gram weight and an emission factor, the final recipe-level CO$_2$e was computed as the sum over its ingredients:
\begin{equation}
    \text{co2e\_kg} = \sum_{i} \left( \text{qty\_amount}_i \times \text{g\_per\_unit}_i \times \frac{\text{co2\_kg\_per\_kg}_i}{1000} \right)
\end{equation}
This yields one score per recipe, stored in \texttt{recipe\_co2.csv} (one row per recipe, 9,997 rows). Across all recipes, the median CO$_2$e is 8.39\,kg CO$_2$eq and the mean is 13.41\,kg CO$_2$eq. Because the gram weights are estimates rather than exact measurements, these values are best read as proportional indicators for comparative ranking rather than absolute measurements. The assumptions underlying each conversion default are catalogued in Appendix~\ref{app:assumptions}.
...
\begin{itemize}
    ...
    \item \textbf{Relational to Graph Exchange:} Flat tabular data (CSVs) was transformed into a graph structure by extracting cleaned nodes (Recipe, Ingredient, Cuisine) and edges (HAS\_INGREDIENT with gram-weight and CO$_2$e attributes) and loading them into the Neo4j Knowledge Graph schema for Cypher querying.
\end{itemize}
...
The three source datasets are mutually incompatible as delivered: recipe quantities are free-text culinary units, while emission factors are per-kilogram rates, and neither side shares a common ingredient vocabulary. A three-stage preprocessing pipeline reconciles them into a single query-ready representation in which every recipe carries an estimated carbon footprint in kg CO$_2$eq: (1) ingredient canonicalisation and CO$_2$e rate assignment (Wolfram primary, OWID fallback, 100\% coverage of the 58 canonical ingredients), (2) gram-weight grounding via USDA SR Legacy portion data, and (3) recipe-level CO$_2$e summation. The end-to-end pipeline is shown in Figure~\ref{fig:pipeline}. This stage required both automated processing and manual curation and was the most complex part of the project; it is documented in full --- including the per-ingredient emission factors, the coverage build-up, the gram-weight tier split, and the recipe-level CO$_2$e formula --- under Section~4 (``How did you integrate the different datasets?'' and ``If you use data exchange in your project''), with the conversion assumptions and ingredient mappings in Appendices~\ref{app:assumptions} and~\ref{app:mappings}. The output is one CO$_2$e score per recipe (median 8.39\,kg CO$_2$eq, mean 13.41\,kg CO$_2$eq), which enriches the \texttt{HAS\_INGREDIENT} edges described below.
...
\begin{figure}[h]
    ...
    \texttt{HAS\_INGREDIENT} edges of the Knowledge Graph (purple). The stages are detailed in
    Section~4.}
    \label{fig:pipeline}
\end{figure}
